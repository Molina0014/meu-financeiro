...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      const { type, category, month, from, to, sort } = req.query;
      ...
      if (month && /^\d{4}-\d{2}$/.test(month)) {
        conditions.push(`date_trunc('month', date) = $${paramIdx++}::date`);
    ...
    if (req.method === 'POST') {
      ...
      if (!type || !VALID_TYPES.includes(type)) {
        return res.status(400).json({ error: `type deve ser: ${VALID_TYPES.join(', ')}` });
      }
      if (!category || !VALID_CATEGORIES.includes(category)) {
    ...
    if (req.method === 'PUT') {
      ...
      if (amount !== undefined) {
        ...
        if (!num || num <= 0) return res.status(400).json({ error: 'amount inválido' });
